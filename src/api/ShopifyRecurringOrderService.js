...
class ShopifyRecurringOrderService {
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "ShopifyRecurringOrder");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "Array<ShopifyRecurringOrder>");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
    ...
    * @param {*} [options] Override http request options.
    */
    update(spaceId, updateRequest, options = {}) {
        ...
        let localVarRequestOptions = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer_1.ObjectSerializer.serialize(updateRequest, "ShopifyRecurringOrderUpdateRequest"),
            timeout: this._timeout * 1000
        };
        this.authentications.default.applyToRequest(localVarRequestOptions);
        if (Object.keys(localVarFormParams).length) {
