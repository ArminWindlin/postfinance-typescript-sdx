...
class InvoiceReconciliationRecordService {
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "InvoiceReconciliationRecord");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "Array<InvoiceReconciliationRecord>");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
    ...
    * @param {*} [options] Override http request options.
    */
    searchForInvoicesByQuery(spaceId, query, options = {}) {
