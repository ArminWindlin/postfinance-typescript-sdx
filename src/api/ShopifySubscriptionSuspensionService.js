...
class ShopifySubscriptionSuspensionService {
    ...
    * @param {*} [options] Override http request options.
    */
    reactivate(spaceId, subscriptionId, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "ShopifySubscriptionSuspension");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "Array<ShopifySubscriptionSuspension>");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
    ...
    * @param {*} [options] Override http request options.
    */
    suspend(spaceId, suspension, options = {}) {
        ...
        let localVarRequestOptions = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer_1.ObjectSerializer.serialize(suspension, "ShopifySubscriptionSuspensionCreate"),
            timeout: this._timeout * 1000
        };
        this.authentications.default.applyToRequest(localVarRequestOptions);
        if (Object.keys(localVarFormParams).length) {
