...
class LegalOrganizationFormService {
                ...
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "Array<LegalOrganizationForm>");
    ...
    */
    country(code, options = {}) {
        ...
        if (code !== undefined) {
            localVarQueryParameters['code'] = ObjectSerializer_1.ObjectSerializer.serialize(code, "string");
        }
        Object.assign(localVarHeaderParams, options.headers);
        let defaultHeaderParams = {
            "x-meta-sdk-version": "3.2.1",
        ...
        return new Promise((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode) {
                        if (response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer_1.ObjectSerializer.deserialize(body, "LegalOrganizationForm");
                            return resolve({ response: response, body: body });
                        }
                        else {
                            let errorObject;
